...
BOOL CTreeCtrlDlg::OnInitDialog()
{
	...
	HTREEITEM hItem1 = m_TreeCtrl.InsertItem(_T("一级节点01"),hRoot);
	m_TreeCtrl.InsertItem(_T("一级节点02"),hRoot);
	m_TreeCtrl.InsertItem(_T("一级节点03"),hRoot);
	hItem = m_TreeCtrl.InsertItem(_T("一级节点04"),hRoot);

	m_TreeCtrl.InsertItem(_T("二级节点011"),hItem1);
...
{
	// TODO: 在此添加控件通知处理程序代码
	m_TreeCtrl.SetItemText(m_TreeCtrl.GetRootItem(),_T("改变后的"));
}
